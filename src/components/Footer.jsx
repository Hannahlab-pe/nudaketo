    ...
    <footer className="bg-nk-choco overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 pt-16 pb-10">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="flex flex-col gap-4">
            <p style={{ fontFamily: "'DM Mono', monospace" }} className="text-nk-gold text-[10px] tracking-[3px]">
              PRODUCTOS
            </p>
            <nav className="flex flex-col gap-2.5">
              {products.map(({ label, href }) => (
                <Link key={label} to={href} className="text-nk-ivory/55 hover:text-nk-ivory text-sm transition-colors duration-200">
                  {label}
                </Link>
              ))}
            </nav>
          ...
          <div className="flex flex-col gap-4">
            ...
            <nav className="flex flex-col gap-2.5">
              {navLinks.map(({ label, href }) => (
                <a key={label} href={href} className="text-nk-ivory/55 hover:text-nk-ivory text-sm transition-colors duration-200">
                  {label}
                </a>
              ))}
            </nav>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="mt-4 pt-4 border-t border-nk-ivory/8">
              <p style={{ fontFamily: "'DM Mono', monospace" }} className="text-nk-gold text-[10px] tracking-[3px] mb-2.5">
                CONTACTO
              </p>
              <a
                href="https://wa.me/51999999999"
                className="text-nk-ivory/55 hover:text-nk-gold text-sm transition-colors flex items-center gap-2"
              >
      ...
      <div className="relative overflow-hidden">
        <p
          ...
          className="font-black text-nk-ivory/6 px-4 lg:px-8 pt-6 pb-0 select-none whitespace-nowrap"
        >
          NUDA KETO
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
        <p style={{ fontFamily: "'DM Mono', monospace" }} className="text-nk-ivory/25 text-[11px] tracking-wider">
          © 2026 NUDA KETO. TODOS LOS DERECHOS RESERVADOS.
        </p>
        <div className="flex items-center gap-4">
          {[
            { label: 'Sin azúcar', href: '/terminos-y-condiciones' },
            { label: 'Gluten free', href: '/terminos-y-condiciones' },
            { label: 'Keto', href: '/terminos-y-condiciones' },
          ].map(({ label, href }) => (
            <Link
              key={label}
              to={href}
              style={{ fontFamily: "'DM Mono', monospace" }}
              className="border border-nk-gold/25 text-nk-gold/60 hover:text-nk-gold hover:border-nk-gold text-[9px] tracking-[1px] px-2.5 py-1 rounded-full transition-colors"
            >
              {label}
            </Link>
          ))}
