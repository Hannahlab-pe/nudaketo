  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-nk-ivory flex flex-col items-center justify-center gap-4 px-5 text-center">
        <p style={{ fontFamily: "'Playfair Display', serif" }} className="text-nk-choco text-2xl font-bold">Mis compras</p>
        <p className="text-nk-muted text-sm">Inicia sesión para ver tu historial de pedidos.</p>
    ...
    <div className="min-h-screen bg-nk-ivory">
      ...
      <div className="max-w-4xl mx-auto px-5 sm:px-6 lg:px-10 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div>
            <p style={{ fontFamily: "'DM Mono', monospace" }} className="text-nk-gold text-[10px] tracking-[4px] mb-2">
        ...
        {loading && <div className="py-24 text-center text-nk-muted">Cargando tus compras...</div>}
        ...
        {!loading && !error && orders.length === 0 && (
          <div className="py-20 text-center flex flex-col items-center gap-4">
            <p style={{ fontFamily: "'Playfair Display', serif" }} className="text-xl text-nk-choco/60">Aún no tienes compras</p>
            <p className="text-nk-muted text-sm">Cuando hagas tu primer pedido, aparecerá aquí.</p>
            <Link to="/tienda" className="bg-nk-choco text-nk-ivory px-6 py-3 rounded-full text-sm font-semibold hover:bg-nk-gold transition-colors">
              Ir a la tienda
            </Link>
          </div>
        )}
